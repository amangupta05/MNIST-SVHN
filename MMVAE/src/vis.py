...
def custom_cmap(n):
    """Create customized colormap for scattered latent plot of n categories.
...
def embed_umap(data):
    """Embed data using UMAP. Data should be on CPU and in numpy format."""
    embedding = UMAP(metric='euclidean', n_neighbors=40, transform_seed=torch.initial_seed())
...
def plot_kls_df(df, filepath):
    ...
    order = sorted(df[df.columns[1]].unique())
    hue_order = sorted(df[df.columns[0]].unique())
    with sns.plotting_context("notebook", font_scale=2.0):
        g = sns.FacetGrid(df, height=12, aspect=2)
        g = g.map(sns.boxplot, df.columns[1], df.columns[2], df.columns[0], palette=cmap_arr[1:], order=order, hue_order=hue_order)
