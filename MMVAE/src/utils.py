...
class Constants(object):
    ...
    logceilc = 88  # largest cuda value such that exp(value) < inf
    logfloorc = -104  # smallest cuda value such that exp(value) > 0
    ...
    if is_multidata(dataB):
        if torch.is_tensor(dataB[0]):
            if torch.is_tensor(dataB[1]):
                return dataB[0].to(device)  # mnist, svhn
            elif is_multidata(dataB[1]):
                return dataB[0].to(device), dataB[1][0].to(device)  # possible future datasets
            else:
                raise RuntimeError(f'Invalid data format {type(dataB[1])} -- check your dataloader!')

        elif is_multidata(dataB[0]):
            return [d.to(device) for d in list(zip(*dataB))[0]]  # mnist-svhn
        else:
            raise RuntimeError(f'Invalid data format {type(dataB[0])} -- check your dataloader!')
    elif torch.is_tensor(dataB):
        return dataB.to(device)
    else:
        raise RuntimeError(f'Invalid data format {type(dataB)} -- check your dataloader!')
...
def pdist(sample_1, sample_2, eps=1e-5):
    ...
    Adapted from the torch-two-sample library.
    """
